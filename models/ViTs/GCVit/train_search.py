...
group.add_argument("--data_len", default=0, type=int, help="size of the dataset")
...
group.add_argument("--damping", default=2e-3, type=float)
group.add_argument("--T", default=20, type=int)
group.add_argument("--beta2", default=0.5, type=float)
group.add_argument("--lr_cov", default=2e-2, type=float)
group.add_argument(
    "--dummy_init",
    action="store_true",
    default=False,
...
def main():
    ...
    if args.log_wandb:
        ...
        if has_wandb:
            opt_lower_out = args.opt.lower()
            if args.amp:
                opt_lower_out = "%s-16" % opt_lower_out
            else:
                opt_lower_out = "%s-32" % opt_lower_out
            if not mixup_active:
                opt_lower_out = "%s-noaug" % opt_lower_out
    ...
    assert use_amp == "native"
    assert args.native_amp is True
    if args.aa.lower() == "none":
        args.aa = None
    else:
        print("aa:", args.aa)
    # assert args.aa is None
    ...
    optim_name = args.opt.lower()
    print(optim_name, "lr:", args.lr, "wt:", args.weight_decay)
    if optim_name == "sgd":
    ...
    elif optim_name.startswith("ifshampoo"):
        if args.amp:
            dtype = torch.bfloat16
            assert use_amp == "native"
        else:
            dtype = torch.float32
        structure = "_".join(optim_name.split("_")[1:])

        assert structure == "dense"
    ...
    elif optim_name.startswith("shampoo"):
        optimizer = DistributedShampoo(
            model.parameters(),
            lr=args.lr,
            betas=(args.momentum, 1.0 - args.lr_cov),
            epsilon=args.damping,
            weight_decay=args.weight_decay,
        ...
        optimizer = MyRmsProp(
            ...
            dummy_scaling = args.dummy_scaling,
        )
    ...
    args.data_len = len(dataset_train)
    ...
    print('here', lr_scheduler, num_epochs )
    ...
    if mixup_active:
        ...
        if args.mixup > 0:
            print("enabled mixup", args.mixup)

        if args.cutmix > 0:
            print("enabled cutmix", args.cutmix)
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.log_wandb and has_wandb:
                optim_name = args.opt.lower()
                lr_aux = optimizer.param_groups[0]["lr"]
                log_data = {
            ...
            print("epoch:", epoch, "test acc1:", eval_metrics["top1"])
