...
class GlobalQueryGen(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.window_size != H or self.window_size != W:
            x = interpolate(
                x, size=(self.window_size, self.window_size), mode="bicubic"
            )
        x = _to_channel_last(x)
        ...
        **kwargs,
        ...
        for level in self.levels:
            for block in level.blocks:
                ...
                relative_position_bias_table_pre = (
                ...
                relative_position_bias_table_pretrained_resized = (
                    torch.nn.functional.interpolate(
                        relative_position_bias_table_pre.permute(1, 0).view(
                            1, nH1, S1, S1
                        ),
                        size=(S2, S2),
                        mode="bicubic",
                ...
                block.attn.relative_position_bias_table = torch.nn.Parameter(
                    relative_position_bias_table_pretrained_resized
                )
