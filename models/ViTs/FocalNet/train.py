...
from scheduler_factory import create_scheduler_v2
...
def main():
    ...
    if args.amp:
        ...
        print("amp is enabled", amp_dtype)
    ...
    if utils.is_primary(args) and args.log_wandb:
        optim_name = args.opt.lower()
        if has_wandb:
            opt_name = "%s" % optim_name
            if args.amp:
                run_name = "%s-%s-16" % (opt_name, socket.gethostname())
            else:
                run_name = "%s-%s-32" % (opt_name, socket.gethostname())
            # wandb.init(
                # project=args.experiment, name=run_name, config=args,
            # )
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            print("epoch:", epoch, "test acc:", save_metric)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if update_idx % args.log_interval == 0:
            ...
            if utils.is_primary(args):
                _logger.info(
                    f"Train-{optim_name}: {epoch} [{update_idx:>4d}/{updates_per_epoch} "
