...
#adapted from https://github.com/dyhan0920/PyramidNet-PyTorch
...
class PyramidNet(nn.Module):
    ...
    def __init__(self, depth, alpha, num_classes, bottleneck=True, small_net=True):
...
def pyramidnet101(num_classes, **kwargs):
    """
    Constructs a NN model.
    """
    model = PyramidNet(depth=101, alpha=48, num_classes=num_classes,
                       bottleneck=True, small_net=True)
    return model


def pyramidnet65(num_classes, **kwargs):
    """
    Constructs a NN model.
    """
    model = PyramidNet(depth=65, alpha=48, num_classes=num_classes,
                       bottleneck=True, small_net=True)
    return model
...
# if __name__ == "__main__":
    ...
    # img = torch.ones([8, 3, 96, 96], device='cuda')
