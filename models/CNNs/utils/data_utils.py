...
def get_transforms(dataset):
    ...
    if (
        dataset.find("imagewoof") >= 0
        or dataset.find("tinyimagenet") >= 0
        or dataset.find("imagenet100") >= 0
    ):
        return None, None
...
def get_dataloader(
    dataset, train_batch_size, test_batch_size, num_workers=2, root="./data"
):
    ...
    if dataset.find("cifar100") >= 0:
        ...
        trainset, testset = get_imagenet(data_path)

    elif dataset.find("imagewoof") >= 0:
